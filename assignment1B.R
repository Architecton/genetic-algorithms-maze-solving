...
if (as.integer(mode) == 1) {
  ...
  maze2B_aux <- maze2B
  fin <- c()
  for (k in 1:num_coins1) {
    res1 <- vector("list", as.numeric(num_coins))
    res1 <- foreach(i=1:num_coins) %dopar% {
      maze2B_auxaux <- maze2B_aux
      maze2B_auxaux[maze2B_auxaux == 'e'] <- ' '
      maze2B_auxaux[maze2B_auxaux == 'c'][i] <- 'e'
      geneticAlgorithm(population_size=as.numeric(population_size), fitness_f=fitness_maze_path, params=list(maze2B_auxaux, rows2B, cols2B), min_len=length(maze2B), max_len=length(maze2B), min_val=0, max_val=4, max_run=as.numeric(max_run), lim_run=as.numeric(lim_run), plot_iterations=plot_it, verbose=verbose) 
    }
    col1 <- map(res1, {function(x) x[[1]]})
    ...
    df$chromosomes <- col2
    idx_max_fitness <- which(df$fitness == max(df$fitness))[1]
    chr_max <- df$chromosomes[idx_max_fitness]
    chr_max <- Filter({function(x) x != 0}, chr_max[[1]])
    chr_max <- chr_max[1:df$steps[idx_max_fitness]]
    fin <- c(fin, chr_max)
    maze2B_aux[maze2B_aux == 's'] <- ' '
    maze2B_aux[maze2B_aux == 'c'][idx_max_fitness] <- 's'
    num_coins <- num_coins - 1
  }
  res <- vector("list", as.numeric(num_iterations))
  res <- foreach(i=1:num_iterations) %dopar% {
    geneticAlgorithm(population_size=as.numeric(population_size), fitness_f=fitness_maze_path, params=list(maze2B_aux, rows2B, cols2B), min_len=length(maze2B), max_len=length(maze2B), min_val=0, max_val=4, max_run=as.numeric(max_run), lim_run=as.numeric(lim_run), plot_iterations=plot_it, verbose=verbose) 
  }
...
fitness_max_chromosome <- mapvalues(fin, c(0, 1, 2, 3, 4), c('O', 'U', 'D', 'L', 'R'), warn_missing = FALSE)
...
# TODO
coinPath <- function(maze, rows, columns) {
  # Enclose in function
  num_coins1 <- 9
  num_coins <- 9
  maze2B_aux <- maze2B
